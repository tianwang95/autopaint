...
class Painting:
    ...
    def random_neighbors(self, n: int, stddev: np.ndarray) -> Tuple[List['Painting'], np.ndarray]:
        """
        Generate some random neighbors for this painting
        """
        _stddev = stddev if isinstance(stddev, float) else np.expand_dims(stddev, axis=0)
        # generate a sample with norm
        sample = np.random.normal(size=[n, *self.params.shape])
        while True:
            new_params = np.expand_dims(self.params, axis=0) + (sample * _stddev)
            should_resample = np.logical_or(new_params > 1.0, new_params < 0.0)
            # never resample angles
            should_resample[:, :, 4] = False
        ...
        new_paintings = [Painting(self.width, self.height, params) for params in sorted_params]
        return new_paintings, sorted_params
...
class Trainer:
    ...
    def train(self, steps: int, checkpoint_freq: int = 10, save_dir: str = None):
        for i in tqdm(range(steps)):
            ...
            if i % checkpoint_freq == 0 and self.save_dir is not None:
                _, surface = self.painting.render()
                surface.write_to_png(self.checkpoint_image_path(f"step_{i}.png"))

    def step(self):
        # generate a new population by getting neighbors for best painting
        population, params = self.painting.random_neighbors(self.population_size, 0.1)

        # weighted sum to find the best
        rewards = 1.0 - np.asarray([self.loss(p.render()[0]) for p in population])
        weights = rewards / np.sum(rewards)
        best_params = np.sum(np.reshape(weights, (-1, 1, 1)) * params, axis=0)
        print(weights)
        print(best_params)
    ...
    def loss(self, source: np.ndarray) -> float:
        ...
        L1 Loss
        """
        return np.sum(np.abs(self.original - source)) / (self.original.shape[0] * self.original.shape[1])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--save-dir", type=str, default=None, help="The directory to save outputs to")
    parser.add_argument("-c", "--config", type=str, help="If provided, it will override any existing options with this config")
